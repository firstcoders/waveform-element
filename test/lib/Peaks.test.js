...
describe('Peaks', () => {
  ...
  it('increases duration by padding emtpy data at the end', () => {
    const peaks1 = new Peaks(peaks1Data);
    expect(peaks1.peaksPerSecond).to.equal(40);
    expect(peaks1.duration).to.equal(73.35);

    const peaks2 = peaks1.setDuration(152.65);
    expect(peaks2.duration).to.equal(152.65);
    expect(peaks2.peaksPerSecond).to.equal(40);
  });

  it('decreass duration truncating peaks', () => {
    const peaks1 = new Peaks(peaks1Data);
    expect(peaks1.peaksPerSecond).to.equal(40);
    expect(peaks1.duration).to.equal(73.35);

    const peaks2 = peaks1.setDuration(30);
    expect(peaks2.duration).to.equal(30);
    expect(peaks2.peaksPerSecond).to.equal(40);
  });
